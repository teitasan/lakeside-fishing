...
| タナ（狙う層） | `E` で仕掛けウインドウ → 水の柱をクリック／ドラッグ（0.5〜30m。キャスト前のみ） |
...
2. **タナを決める**：`E` で仕掛けウインドウを開き、**エサを沈める深さ**を 0.5〜30m から選びます（エサの種類とは独立）。
   水の柱をクリック／ドラッグするだけで、その層に居る魚が一覧で出ます（未発見は `???`）。
   狙う魚の生息層に合わせると重み ×1.68、外すと ×0.08。**エサ × タナ**の組み合わせで釣れる魚が変わります。
   設定が水深より深ければ底べたになり、HUD が「→ 底 ○m」に変わります。**キャスト後は変更できません**（回収してから結び直す）。
...
- HUD の水深と図鑑の生息層を見て、**タナを合わせる**のが基本。エサはその層にいる魚の好みで選びます。
...
- **タナ（狙う層）を `E` の仕掛けウインドウで設定**。エサとは独立で、エサ × タナで釣れる魚が変わります
...
- **内部状態**：fps / フレーム時間 / draw call / 三角形数、プレイヤー座標・地面の高さ・勾配・
  桟橋ローカル座標・障害物との接触、釣りの状態機械（state・アタリまでの残り時間・タナ 設定/実効）、
