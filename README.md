...
| 図鑑 | `Q`（**さかな** / **地形** の 2 タブ） |
| 視点 | **一人称に固定**（釣り人のモデルを作り直しているあいだの暫定。`src/game.js` の `FPV_ONLY` を `false` に戻すと、ホイールでの三人称／一人称の切り替えが復活します） |
...
1. **キャスト**：まず**マウスで狙う**（見下ろすほど近く、水平にするほど遠く。水面の輪が狙い点）。
   ...
   HUD に**狙い距離とその地点の水深**が出るので、投げる前にポイントを下見できます。**足元 1.7m から**同じ操作で狙え、
   最大飛距離はロッドで変わります（竹の釣り竿は 10m、伝説の竿は 84m）。
...
- **エピック（チョウザメ・アリゲーターガー）はレベルが足りなくても極低確率で掛かります**
  （深場・夜ほど高く、Lv1 の淵で 100 回に 1 回ほど）。獲れるかは道具次第。
  **湖の主・イトウ**だけはレベル解禁（Lv6〜11）が必要です。
- 深い淵で夜・雨、そして高級エサ。**湖の主**や幻の**イトウ**が眠っているという噂です。

## 収録内容

- 魚・生きもの 30 種（コモン〜レジェンド。エビ・ザリガニ・カニも）＋ ゴミ 4 種 / 図鑑・自己記録・実績 9 種
- **湖の測量図**（`M`）：440m 四方を 72×72 の格子（1 マス 6.1m）で持ち、**歩いた所と仕掛けを落とした所（どちらも半径 30m）だけ**
...
- **一人称／水中**の 2 カメラ。一人称ではロッドを手前に持ち替えたように寝かせて構えます。
  釣り人のモデルを作り直しているあいだは体を出していないので、竿とリールだけが宙に浮いて見えます
  （ボーンは動き続けていて竿を手の位置に置いているので、狙いや飛距離の計算は変わりません）。
  `src/game.js` の `FPV_ONLY` を `false` に戻すと、三人称と体の表示がそのまま復活します
...
`motion-editor.html` をブラウザで開くと、釣り人の姿勢をスライダーで調整できます。

- **ゲーム本体と同じ `Angler` クラスを動かしている**ので、ここで見えるものがそのままゲームの動きです
- 場面（待機／ためる／アタリ待ち／ファイト／取り込み）を切り替えて、竿のピッチ・手の位置・前傾・
  肘の張り出し・指の曲げ・歩きの再生倍率などを調整できます
- **「保存してゲームに反映」**で `localStorage` に保存されます。ゲームを別タブで開いていれば**その場で反映**されます
- **「コードをコピー」**で `src/angler.js` の `TUNING` に貼れる形の JS を出します（恒久的に残す場合はこちら）
- 「JSON 書き出し」「初期値に戻す」もあります

歩きの再生倍率を触ると、画面左上に**足の滑り**（アニメの歩幅から出る「滑らない速さ」と
ゲームの移動速度の比）が出ます。
