/* ===========================================================
   ...
   =========================================================== */
import { TAU, clamp01, lerp, lodForList } from './util.js?v=20260830-zone5';
 ...
 *
 * 近景の面積は半径の 2 乗で効くので、ここが森の予算をほぼ決める。
 * 岸の 24 箇所から実測した森の三角形（2000 本, high）:
 *   near=34 → 平均 333k / 最悪 410k
 *   near=48 → 平均 437k / 最悪 552k
 *   near=58 → 平均 549k / 最悪 653k
 * 中景の葉を 1/5 → 1/7 に減らして浮いた分を、そのまま near に回して 48 にした
 * （中景の葉を減らす前の near=34 が 平均 445k / 最悪 549k だったので同予算）。
 */
/* 2026-08-28: 近景を 48 → 85m へ広げた。近づいたときの «形が変わる»
   切り替わりを目に付きにくい距離へ押し出すため。面積は 3.1 倍になる */
/* 木が 2000 本だった頃は近景 85m まで «本気のジオメトリ» で回せた。
   28000 本になると同じしきい値で近景 300 本・中景 1100 本＝ 12M 三角形に
   なり、これだけで GPU の 4 割を持っていく。下草が足元を埋めたので、
   近景の «枝が 3D かどうか» は前ほど効かない。しきい値を詰めて、
   浮いたぶんを本数に回す */
export const LOD_DIST = [58, 145];
