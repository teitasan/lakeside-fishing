...
/* この 2 つは TENSION_TEMPO（下の方・0.5）と必ずセットで動かすこと。
   張力の上下だけを半分にして距離をそのままにすると、難易度は変わらないまま
   「張力が満タンになる前に魚が寄り切ってしまう」になる。実際そうなっていて、
   初期竿で 5m に掛けた魚は 1.6 秒・一度も糸を送らずに寄っていた
   （巻き 4.12m/s を実測）。両方に同じ 0.5 を掛けてあるので、
   押す／離すの比率＝難易度はそのままで、テンポだけが半分になる */
const REEL_MPS = 2.3;
const LINEOUT_MPS = 0.35;
...
/* 張力の上下を遅くする倍率。REEL_MPS / LINEOUT_MPS にも同じ倍率が入っていて、
   3 つセットで「難易度は変えずファイトのテンポだけ落とす」になっている。
   ここだけ動かすと距離と張力の進み方がずれる（上の REEL_MPS のコメント参照） */
