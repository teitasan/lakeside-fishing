...
export class MultiplayerFishingSync {
  ...
  onFishSnapshot(items) {
    ...
    if (!g.hookFish && g.fs === 'wait') {
      const mine = items.find((f) => f.targetBaitId === `b:${this.mp.id}` && (f.state === 'approaching' || f.ownerPlayerId === this.mp.id));
      if (mine) { const local = g.sharedFish?.get(mine.id); if (local) { local.state = mine.ownerPlayerId === this.mp.id ? 'nibble' : 'approach'; g.fishing.beginApproach(local, { source: 'server' }); } }
    }
  ...
  setBaitFromGame() {
    const g = this.game; if (!this.mp?.connected || g.fs !== 'wait') return;
    const x = g.bobber.x, z = g.bobber.z;
    this.mp.setBait({
      x, y: g.baitY, z,
      baitType: g.bait.id, rigLayer: g.rigLayer.id,
      rodType: g.rod.id, level: g.state.level,
      bed: g.terrain.bedAt(x, z).kind,
      nearStruct: !!g.terrain.structureNear(x, z, 4.5),
      castAcc: g.castAcc ?? 0, castPower: g.castPower ?? 0,
    });
  }
  sendFightPosition() {
    const g = this.game; if (g.fs !== 'fight' || !g.hookFish?.networkId || !this.mp?.connected) return;
    const now = performance.now(); if (now - this.lastFightSentAt < 100) return; this.lastFightSentAt = now;
    const p = g.hookFish.pos; this.mp.fightUpdate(g.hookFish.networkId, p.x, p.y, p.z);
  }
}
