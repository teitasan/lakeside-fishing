...
export class MultiplayerRoom {
  ...
  _restore(ws) {
    ...
    if (p) return p;
    let att = null; try { att = ws.deserializeAttachment(); } catch (e) { /* noop */ }
    if (!att?.joined) return null;
    p = { id: att.id, name: att.name, x: 0, y: 0, z: 0, yaw: 0, a: 'idle', fresh: true, lastMsg: 0 };
  ...
  _broadcast(obj, except = null) {
    ...
    for (const { sock } of this._joined()) if (sock !== except) { try { sock.send(s); } catch (e) { /* noop */ } }
  }
  ...
  _startLoop() {
    ...
    this.loop = setInterval(() => {
      ...
      if (now - this.lastSnapshot >= 100) {
        ...
        this._broadcast({ t: 'fish_snapshot', fish: this.world.snapshot() });
      }
    }, 100);
  }
  ...
  async webSocketMessage(ws, raw) {
    ...
    if (msg.t === 'join') {
      ...
      const p = { id, name, x: 0, y: 0, z: 0, yaw: 0, a: 'idle', fresh: true, lastMsg: 0 };
    ...
    const now = Date.now();
    if (now - p.lastMsg < MIN_STATE_MS && ['s', 'fight'].includes(msg.t)) return;
    p.lastMsg = now;
    ...
    if (msg.t === 'fight') { this.world.fightUpdate(p.id, String(msg.fishId || ''), msg); return; }
    if (msg.t === 'fight_end') {
  ...
  _drop(ws) {
    const p = this._restore(ws); this.players.delete(ws);
