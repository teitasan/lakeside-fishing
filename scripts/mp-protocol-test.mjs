...
function connect(name) {
  return new Promise((resolve, reject) => {
    ...
    ws.onopen = () => { ws.send(JSON.stringify({ t: 'join', v: PROTO, name })); const next = (timeoutMs = 5000) => new Promise((res, rej) => { if (queue.length) return res(queue.shift()); const timer = setTimeout(() => rej(new Error(`timeout ${name}`)), timeoutMs); waiters.push((m) => { clearTimeout(timer); res(m); }); }); const nextType = async (type, timeoutMs = 6000) => { const end = Date.now() + timeoutMs; while (Date.now() < end) { const m = await next(Math.max(100, end - Date.now())); if (m.t === type) return m; } throw new Error(`timeout ${name}:${type}`); }; resolve({ ws, next, nextType, send: (o) => ws.send(JSON.stringify(o)) }); };
    ws.onerror = () => reject(new Error(`connect failed: ${name}`));
  });
}
const a = await connect('Alice'), wa = await a.nextType('welcome');
if (typeof wa.clock !== 'number' || !Array.isArray(wa.fish) || !wa.weather || !Array.isArray(wa.visuals)) fail('welcome world state不足'); ok('welcomeに時刻・天候・共有魚snapshotあり');
a.send({ t: 's', x: 10.5, y: 1.2, z: -3.25, yaw: 0.5, a: 'walk' }); await new Promise((r) => setTimeout(r, 250));
const b = await connect('ボブ'), wb = await b.nextType('welcome'); const alice = wb.players.find((p) => p.id === wa.id); if (!alice || alice.x !== 10.5) fail('参加時player snapshot不正'); await a.nextType('join'); ok('2人参加・位置snapshot・日本語名');
a.send({ t: 's', x: 12, y: 1.2, z: -3, yaw: 1, a: 'run' }); const state = await b.nextType('s'); if (state.id !== wa.id || state.x !== 12) fail('位置relay不正'); ok('位置relay');
